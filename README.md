...
- `GET /map/server` — returns the public map server identity for `dyson-sphere-ring-1`, combining the map database status, public digest authentication, `server_role: "map_database_reference_anchor"`, `operation: "Seconds_Of_Intelligence"`, `tick_rate_hz: 1`, canonical public URLs derived from `PUBLIC_BASE_URL`, and digest/verification fields from `map_assets`. The embedded authentication record uses the same public metadata filtering as `/map/authenticate/:mapId`, so private metadata remains hidden from public callers. This route depends on seeded map digest data in `map_assets`; if the map asset has not been seeded yet, the service still returns HTTP 200 with `map_database`, `canonical_public_urls`, `authentication_status: "map_asset_not_seeded"`, and a `next_step` directing operators to run `npm run migrate`, but omits the digest and embedded `authentication` fields until the seed data exists.
