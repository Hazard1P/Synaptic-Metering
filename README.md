...
- `GET /map/dyson-sphere-ring-1` — public, human-readable searchable URL for the Dyson-Sphere Ring-1 map database anchor. It renders canonical metadata from `PUBLIC_BASE_URL`, links to `/map/database?anchor_id=dyson-sphere-ring-1`, `/map/authenticate/dyson-sphere-ring-1`, and `/public/dyson-sphere-ring-1-map.svg`, and is suitable for linking from SynapticSystems.ca search/navigation surfaces.
